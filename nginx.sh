...
define host{
        ...
        alias                   serverb1
        address                 172.25.15.11
        }
define hostgroup{
        hostgroup_name  linux-server 
        alias           linux-server
        members         serverb.pod15.example.com     
        }
# 定义监控服务
define service{
        use generic-service
        host_name serverb.pod15.example.com
...
ENM
sed -i '/^172.25.15.10'/d /root/.ssh/known_hosts
expect << ENO 
spawn  scp aaaaaa.sh 172.25.15.10:/root/
expect "(yes/no)?"
send "yes\r"
expect "password:"
send "uplooking\r"
